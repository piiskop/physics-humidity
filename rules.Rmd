...
&= \frac{\frac{W \times p}{W + `r M_w_TO_M_d`}}{p_\text{sat}(w)} = \frac{W \times p}{(W + `r M_w_TO_M_d`) \cdot p_\text{sat}(w)}
(\#eq:relative-humidity)
\end{align}
```

[@American_Society_of_Heating_Refrigerating_and_Air-Conditioning_Engineers2017-im, p. 14], [@relative].
...
Avaldan suhtelise niiskuse valemist \@ref(eq:relative-humidity) veeauru osarõhu ja asendan selle niiskuse suhte valemisse \@ref(eq:humidity-ratio):

```{=tex}
\begin{align}
W := \frac{`r M_w_TO_M_d` \times RH \times p_\text{sat}(w)}{p - RH \times p_\text{sat}(w)}.
...
```{=tex}
\begin{align}
H :&= H_d + H_w = m_d \cdot h_d + m_w \cdot h_w.
(\#eq:sum-of-enthalpy)
\end{align}
``` 
...
```{=tex}
\begin{align}
\frac{H}{m_d} &= h_d + \frac{m_w}{m_d} \cdot h_w
h :&= h_d + W \cdot h_w.
...
```{=tex}
\begin{align}
m_d \cdot c_p(in) \cdot \Delta(T) + m_f \cdot c_p(f) \cdot \Delta(T) &= m_d \cdot c_p(\text{sat}) \cdot \Delta(T)\\
m_d \cdot h_\text{in} + m_f \cdot h_f &= m_d \cdot h_\text{sat}\\
\cancel{m_d} \cdot h_\text{in} + \cancel{m_d} \cdot (W_\text{out} - W_\text{in}) \cdot h_f &= \cancel{m_d} \cdot h_\text{sat}\\
h_d(\text{in}) + W_\text{in} \cdot h_w(\text{in}) + (W_\text{out} - W_\text{in}) \cdot h_f &= h_d(\text{sat}) + W_\text{sat} \cdot h_w(\text{sat})\\
c_p(\text{in}) \cdot T_d + W_\text{in} \cdot h_w(\text{in}) + (W_\text{out} - W_\text{in}) \cdot h_f &= c_p(\text{sat}) \cdot T_\text{sat} + W_\text{sat} \cdot h_w(\text{sat})
(\#eq:energy-balance)
\end{align}
``` 

Avaldan energia tasakaalu valemist \@ref(eq:energy-balance) niiskuse suhte:
...
&= c_p(\text{dry air}) \cdot T3 + W \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot T)
...
```{=tex}
\begin{align}
T_dp <- 6.54 + 14.526 * ln(p_w) + .7389 * (ln(p_w))^2 + .09486 * (ln(p_w))^3 + .4569 * p_w^.1984 - 273.15.
