...
W_\text{in} \cdot m_\text{d} + m_f &= W_\text{out} \cdot m_\text{d}\\
m_\text{f} :&= m_\text{d} \cdot (W_\text{out} - W_\text{in})
...
&= c_p(\text{dry air}) \cdot (T - \qty{`r T_0`}{\kelvin} + W \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T - \qty{`r T_0`}{\kelvin}))
...
Kirjutan energia tasakaalu valemi \@ref(eq:energy-balance) lahti:

```{=tex}
\begin{align}
c_p(\text{in}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) + W \cdot h_w(\text{in}) + W_\text{sat} \cdot h_f - W \cdot h_f\\
= c_p(\text{sat}) \cdot (T_\text{sat} - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot h_w(\text{sat}).
(\#eq:energy-balance-expanded)
\end{align}
```

Avaldan laiendatud energia tasakaalu valemist \@ref(eq:energy-balance-expanded) niiskuse suhte, millesse asendan aurustumissoojuse vastavast valemist \@ref(eq:enthalpy-of-vaporization) kasutades erientalpia valemit \@ref(eq:specific-enthalpy):


```{=tex}
\begin{align}
W \cdot h_w(\text{in}) - W \cdot h_f &= c_p(\text{sat}) \cdot (T_\text{sat} - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot h_w(\text{sat}) - c_p(\text{in}) \cdot (T_d - \qty{`r T_0`}{\kelvin})\\
- W_\text{sat} \cdot h_f\\
W :&= \frac{c_p(\text{sat}) \cdot (T_\text{sat} - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot (h_w(\text{sat}) - h_f) - c_p(\text{in}) \cdot (T_d - \qty{`r T_0`}{\kelvin})}{h_w(\text{in}) - h_f}\\
W :&= \frac{c_p(\text{sat}) \cdot (T_\text{sat} - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot \Delta_\text{vap}(h) - c_p(\text{in}) \cdot (T_d - \qty{`r T_0`}{\kelvin})}{h_w(\text{in}) - h_f}.
(\#eq:humidity-ratio-from-energy-balance)
\end{align}
```

Selgitan niiskuse suhte valemi \@ref(eq:humidity-ratio-from-energy-balance) osi:

```{=tex}
\begin{align}
\underbrace{c_p(\text{sat}) \cdot (T_\text{sat} - \qty{`r T_0`}{\kelvin}) - c_p(\text{in}) \cdot (T_d - \qty{`r T_0`}{\kelvin})}_{\text{mõistlik soojuskadu õhus}} &= \underbrace{W_\text{sat} \cdot \Delta_\text{vap}(h) - W \cdot (h_f - h_w(\text{in}))}_{\text{aurustumine}}
...
Olin tükk aega kimpus, millist soojusmahtuvuse väärtust kasutada, kui see sõltub temperatuurist.
...
&= c_p(\text{dry air}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) &+ W \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_d - \qty{`r T_0`}{\kelvin})\\
&&+ (W_\text{sat} - W) \cdot c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})\\
&= c_p(\text{dry air}) \cdot (T_w - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_w - \qty{`r T_0`}{\kelvin})
...
\begin{align}
c_p(\text{dry air}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) + W \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_d - \qty{`r T_0`}{\kelvin}))\\
+ W_\text{sat} \cdot c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin}) - W \cdot c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})\\
= c_p(\text{dry air}) \cdot (T_w - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_w - \qty{`r T_0`}{\kelvin}))\\
W \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) - c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})) \\
= c_p(\text{dry air}) \cdot (T_w - \qty{`r T_0`}{\kelvin}) + W_\text{sat} \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_w - \qty{`r T_0`}{\kelvin}))\\
- c_p(\text{dry air}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) - W_\text{sat} \cdot c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})\\
= c_p(\text{dry air}) \cdot (T_w - T_d) + W_\text{sat} \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_w - \qty{`r T_0`}{\kelvin}) - c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin}))\\
W := \frac{c_p(\text{dry air}) \cdot (T_w - T_d) + W_\text{sat} \cdot (\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_w - \qty{`r T_0`}{\kelvin}) - c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})}{\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) - c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})}\\
W := \frac{c_p(\text{dry air}) \cdot (T_w - T_d) + W_\text{sat} \cdot (\Delta_\text{vap}(h) + (c_p(\text{sat, steam}) - c_p(\text{sat, liquid water})) \cdot (T_w - \qty{`r T_0`}{\kelvin}))}{\Delta_\text{vap}(h) + c_p(\text{sat, steam}) \cdot (T_d - \qty{`r T_0`}{\kelvin}) - c_p(\text{sat, liquid water}) \cdot (T_w - \qty{`r T_0`}{\kelvin})}
