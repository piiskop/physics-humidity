...
#' This function calculates the saturation pressure.
#' 
#' @param T the absolute temperature
#' 
#' @returns the saturation pressure of the dry bulb
calculate_p_sat_in_inbook <- function(T) {
  return(610.8 * exp((17.27 * (T - T_0)) / (T + T_p_sat)))
}
...
Tegeliku rõhu arvutamiseks saab samuti kasutada nii valemit \@ref(eq:saturation-pressure-over-liquid-h2o) kui \@ref(eq:saturation-pressure-in-inbook), kui on teada kastepunkt.
...
calculate_p_a <- function(p_sat_at_T_w, gamma, T_d, T_w) {
  return(p_sat_at_T_w - gamma * (T_d - T_w))
}
...
calculate_W_saturated <- function(T_d) {
  ...
  for (index in length(T_d)) {
    
    if (T_d[index] < 0) {
      stop(paste("The absolute temperature can't be negative. The current absolute temperature is", T_d[index], "K at the position", index, "."))
    }
...
Avaldan laiendatud energia tasakaalu valemist \@ref(eq:energy-balance-expanded) niiskuse suhte, millesse asendan aurustumissoojuse vastavast valemist \@ref(eq:enthalpy-of-vaporization) leheküljel $\pageref{eq:enthalpy-of-vaporization}$ kasutades erientalpia valemit \@ref(eq:specific-enthalpy):
...
#'
#' @export
calculate_W <- function(T_d, T_w) {

  for (index in length(T_d)) {
    
    if (T_w[index] > T_d[index]) {
      stop(paste("Wet bulb temperature is above dry bulb temperature. The current wet bulb temperature is", T_w[index], "K and the current dry bulb temperature is", T_d[index], "K at the position", index, "."))
    }
...
calculate_Tdp <- function(W) {
  ...
  alpha <- log(p_w)
  T_dp <- 6.54 + 14.526 * alpha + .7389 * alpha^2 + .09486 * alpha^3 + .4569 * p_w^.1984 + T_0
  return(T_dp)
}
...
```{=tex}
\begin{align}
p_\text{w} := \frac{p \cdot W}{`r M_w_TO_M_d` + W}.
...
```{=tex}
\begin{align}
T_\text{dp} :&= (6.54 + 14.526 \times ln(\frac{p_\text{w}}{10^3 \times \unit{\pascal}}) + .7389 \times (ln(\frac{p_\text{w}}{10^3 \times \unit{\pascal}}))^2 + .09486 \times (ln(\frac{p_\text{w}}{10^3 \times \unit{\pascal}}))^3\\
